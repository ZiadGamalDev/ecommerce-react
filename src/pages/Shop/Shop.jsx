...
export default function Shop() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // Fetch categories
                const catRes = await fetch("https://e-commerce-api-tau-five.vercel.app/category");
                const catData = await catRes.json();

                // Fetch brands
                const brandRes = await fetch("https://e-commerce-api-tau-five.vercel.app/brand");
                ...
                if (productData.minPrice !== undefined && productData.maxPrice !== undefined) {
    ...
    useEffect(() => {
        ...
        if (selectedBrands.length > 0) {
            result = result.filter((product) => selectedBrands.includes(product.brand?.slug));
        }

        // (D) Filter by price range
        if (newPriceRange.length === 2) {
            const [minPrice, maxPrice] = newPriceRange;
            result = result.filter((product) => product.finalPrice >= minPrice && product.finalPrice <= maxPrice);
        }
